...
public abstract class PiezaAjedrez extends Pieza {
	...
	public final boolean puedeMover (int toFila, char toColumna){
		return movimientoDentroLimites(toFila, (int) (toColumna) - 97) && (toFila != getFilaAjedrez() || toColumna != getColumnaAjedrez());
	}
...
//	métodos auxiliares
	...
	protected void casillasVisitables (IList<Casilla> resultado,int incFila, int incColumna){
		...
		for (int i = getFila() + incFila, j = getColumna() + incColumna; movimientoDentroLimites(i, j) && !chocado; i += incFila, j += incColumna) {
