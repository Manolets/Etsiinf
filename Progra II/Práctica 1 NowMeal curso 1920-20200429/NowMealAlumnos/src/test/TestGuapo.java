...
public class TestGuapo {
 ...
 /**
  * Test de add (a�adir transporte al GestionRepartoLocal).
  * 
 ...
 public void testAdd() {
  ...
  IList<String> motosEnGR ;
  IList<String> furgosEnGR ;
  ...
  for (int i = 0; i <NUM_VEHICULOS; i++){
   ...
   assertEquals("El n�mero de motos en espera no coincide", i+1, motosEnGR.size());   
   
   gestor.add(furgonetas[i]);
   furgosEnGR = gestor.getCodFurgoDisponibles();   
   assertEquals("El n�mero de furgonetas en espera no coincide", i+1, furgosEnGR.size());
   
   for (int j=0;j<i;j++) {
    assertEquals("El c�digo de las motos en espera no coincide", motos[j].getCodigo(), motosEnGR.get(j));
    assertEquals("El c�digo de las furgonetas en espera no coincide", furgonetas[j].getCodigo(), furgosEnGR.get(j));
   }
  }
  ...
  * porque no hay transportes disponibles. Se comprueban los datos y el orden de inserci�n en las colas.</p>
  */
 @Test
 public void testAsignarPedido0() {
  ...
  for (int i = 0; i <NUM_IT; i++){
   ...
   assertEquals("El n�mero de pedidos en espera de moto no coincide", i+1, pedMoto.size());   
   
   IList<String[]> pedFurgo = gestor.getCodEsperandoFurgo(); 
   assertEquals("El n�mero de pedidos en espera de furgoneta no coincide", i+1, pedFurgo.size());
   
   for (int j=0;j<i;j++) {
    assertEquals("El c�digo del cliente del pedido para moto no coincide",pedidosPequenos[j].getCliente().getCodigo(),pedMoto.get(j)[0]);
    assertEquals("El c�digo del restaurante del pedido para moto no coincide",pedidosPequenos[j].getRestaurante().getCodigo(),pedMoto.get(j)[1]);
    
    assertEquals("El c�digo del cliente del pedido para furgoneta no coincide",pedidosGrandes[j].getCliente().getCodigo(),pedFurgo.get(j)[0]);
    assertEquals("El c�digo del restaurante del pedido para furgoneta no coincide",pedidosGrandes[j].getRestaurante().getCodigo(),pedFurgo.get(j)[1]);
  ...
  * FurgonetaSubcontratada y Pedido, y el m�todo add de GestionRepartoLocal.</p>
  * 
  * <p>Presupone que el m�todo coste de Pedido funciona bien.</p>
  * 
 ...
 public void testAsignarPedido1() {
  ...
  for (int i = 0; i <NUM_IT; i++){
   ...
   IList<String[]> pedMoto = gestor.getCodEsperandoMoto();
   assertEquals("El n�mero de pedidos en espera de moto no coincide", i, pedMoto.size());   
   
   IList<String[]> pedFurgo = gestor.getCodEsperandoFurgo(); 
   assertEquals("El n�mero de pedidos en espera de furgoneta no coincide", i<1?i=0:i-1, pedFurgo.size());
   
   for (int j=1;j<i;j++) {
    assertEquals("El c�digo del cliente del pedido para moto no coincide",pedidosPequenos[j].getCliente().getCodigo(),pedMoto.get(j-1)[0]);
    assertEquals("El c�digo del restaurante del pedido para moto no coincide",pedidosPequenos[j].getRestaurante().getCodigo(),pedMoto.get(j-1)[1]);
    
    if (j>=2){
     assertEquals("El c�digo del cliente del pedido para furgoneta no coincide",pedidosGrandes[j].getCliente().getCodigo(),pedFurgo.get(j-2)[0]);
     assertEquals("El c�digo del restaurante del pedido para furgoneta no coincide",pedidosGrandes[j].getRestaurante().getCodigo(),pedFurgo.get(j-2)[1]);
    }
   } 
  }
 }
  ...
  * porque no quedan transportes disponibles. Se comprueba la correcta asignaci�n de  
  * transportes a pedidos (por coste min) y se comprueban los 
  * datos y el orden de inserci�n en las colas de pedidos pendientes.</p>
  */
 @Test
 public void testAsignarPedido2() {
  ...
  // se crean transportes y se a�aden al gestor local
  ...
  for (int i = 0; i <NUM_IT; i++){
   ...
   IList<String> motosLibres = gestor.getCodMotosDisponibles();
   assertEquals("El n�mero de motos disponibles no coincide", NUM_TRA-i>0?NUM_TRA-i:0, motosLibres.size()); 
   IList<String> furgosLibres = gestor.getCodFurgoDisponibles();
   assertEquals("El n�mero de furgonetas disponibles no coincide", NUM_TRA-i>0?NUM_TRA-i:0, furgosLibres.size()); 
   
   // si quedan motos libres, se asigna a un pedido peque�o
   // cuando NUM_TRA-(i+1)>0, ya no quedar�n motos libres
   if (motosLibres.size()>0){
    ...
    assertEquals("El n�mero de motos disponibles no coincide", NUM_TRA-(i+1)>0?NUM_TRA-(i+1):0, motosLibres.size());  
    for (int j=0;j<motosLibres.size();j++) {
     assertNotEquals("Esta moto no deber�a estar libre", motoAAsignar, motosLibres.get(j));  
     
    }
   }else{ 
    // no queda ninguna moto libre, as� que se comprueba que se encola bien el pedido
    gestor.asignarPedido(pedidosPequenos[i]);
    IList<String[]> pedMoto = gestor.getCodEsperandoMoto(); 
    assertEquals("El n�mero de pedidos en espera de moto no coincide", i-NUM_TRA+1, pedMoto.size());
    
   }
   
   // si quedan furgos libres, se asigna a un pedido grande
   // cuando NUM_TRA-(i+1)>0, ya no quedar�n furgos libres
   if (furgosLibres.size()>0){
    ...
    assertEquals("El n�mero de furonetas disponibles no coincide", NUM_TRA-(i+1)>0?NUM_TRA-(i+1):0, furgosLibres.size());  
    for (int j=0;j<furgosLibres.size();j++) {
     assertNotEquals("Esta furgoneta no deber�a estar libre", furgoAAsignar, furgosLibres.get(j));  
    ...
    gestor.asignarPedido(pedidosGrandes[i]);
    IList<String[]> pedFurgo = gestor.getCodEsperandoFurgo(); 
    assertEquals("El n�mero de pedidos en espera de furgoneta no coincide", i-NUM_TRA+1, pedFurgo.size());
  ...
  * 
  * <p>Utiliza los constructores de GestionRepartoLocal, Moto, FurgonetaPropia, 
  * FurgonetaSubcontratada y Pedido, y los m�todos asignarPedido y add de GestionRepartoLocal.</p>
  *  
  * <p>Se crea un transporte de cada tipo y se crean pedidos para motos y furgonetas.
 ...
 public void testNotificarEntregaPedido0() throws PedidoSinTransporteAsignado {
  ...
  // se crean pedidos, se asignan a los 3 transportes y los dem�s se encolan
  for (int i = 0; i <NUM_IT; i++){
   Cliente cliente = new Cliente("C"+i);
   mapa.addObjetoEstatico(cliente.getCodigo(), new PosicionXY(100+i,100+i));  
   Restaurante restaurante = new Restaurante("R"+i);
   ...
   IList<String[]> pedFurgo = gestor.getCodEsperandoFurgo(); 
   assertEquals("El n�mero de pedidos en espera de furgoneta no coincide", i-1>0?i-1:0, pedFurgo.size());
  ...
  for (int i = 1; i <NUM_IT; i++){
   ...
   IList<String[]> pedMoto = gestor.getCodEsperandoMoto();
   assertEquals("El n�mero de pedidos en espera de moto no coincide", NUM_IT-i-1, pedMoto.size());   
   
   IList<String[]> pedFurgo = gestor.getCodEsperandoMoto(); 
   assertEquals("El n�mero de pedidos en espera de furgoneta no coincide", NUM_IT-i-1, pedFurgo.size());
   
   for (int j=i+1;j<NUM_IT;j++) {
    assertEquals("El c�digo del cliente del pedido para moto no coincide",pedidosPequenos[j].getCliente().getCodigo(),pedMoto.get(j-i-1)[0]);
    assertEquals("El c�digo del restaurante del pedido para moto no coincide",pedidosPequenos[j].getRestaurante().getCodigo(),pedMoto.get(j-i-1)[1]);
    
    assertEquals("El c�digo del cliente del pedido para furgoneta no coincide",pedidosGrandes[j].getCliente().getCodigo(),pedFurgo.get(j-i-1)[0]);
    assertEquals("El c�digo del restaurante del pedido para furgoneta no coincide",pedidosGrandes[j].getRestaurante().getCodigo(),pedFurgo.get(j-i-1)[1]);
  ...
  * 
  * <p>Utiliza los constructores de GestionRepartoLocal, Moto, FurgonetaPropia, 
  * y Pedido, y los m�todos asignarPedido y add de GestionRepartoLocal.</p>
  * 
 ...
 public void testNotificarEntregaPedido1() throws PedidoSinTransporteAsignado {
  ...
  // en cada iter, se asignan un pedido peque�o y otro grande, y se notifican sus entregas
  ...
  for (int i = 0; i <NUM_IT; i++){
   ...
   IList<String[]> pedMoto = gestor.getCodEsperandoMoto();
   assertEquals("El n�mero de pedidos en espera de moto no coincide", 0, pedMoto.size());   
   IList<String> motosDisp = gestor.getCodMotosDisponibles();
   assertEquals("El n�mero de motos disponibles no coincide", motos.length-1, motosDisp.size());   
   
   gestor.asignarPedido(pedidosGrandes[i]); 
   
   IList<String[]> pedFurgo = gestor.getCodEsperandoFurgo(); 
   assertEquals("El n�mero de pedidos en espera de furgoneta no coincide", 0, pedFurgo.size());
   IList<String> fusgosDisp = gestor.getCodFurgoDisponibles(); 
   assertEquals("El n�mero de furgonetas disponibles no coincide", furgos.length-1, fusgosDisp.size());   
   
   for (int j=0;j<motos.length-1;j++) {
    assertNotEquals("Esa moto no deber�a estar libre, iteracion "+i,motos[i%3].getCodigo(),motosDisp.get(j));
    assertNotEquals("Esa furgoneta no deber�a estar libre, iteracion "+i,furgos[i%3].getCodigo(),fusgosDisp.get(j));
   }
   
   gestor.notificarEntregaPedido(pedidosPequenos[i]);
   motosDisp = gestor.getCodMotosDisponibles();
   assertEquals("El n�mero de pedidos en espera de moto no coincide", motos.length, motosDisp.size());   
   gestor.notificarEntregaPedido(pedidosGrandes[i]);   
   fusgosDisp = gestor.getCodFurgoDisponibles(); 
   assertEquals("El n�mero de pedidos en espera de furgoneta no coincide", furgos.length, fusgosDisp.size());
  }
 }
 /**
  ...
  * que nunca hay m�s de un pedido pendiente de cada tipo. Se comprueba que se actualizan correctamente las colas de pedidos pendientes tras
 ...
 public void testNotificarEntregaPedido2() throws PedidoSinTransporteAsignado {
  ...
  // los 3 m�s antiguos sin entregar, por tanto nunca hay m�s de un pedido pendiente de cada tipo
  ...
  for (int i = 0; i <NUM_IT; i++){
   ...
   IList<String[]> pedMoto = gestor.getCodEsperandoMoto();
   assertEquals("El n�mero de pedidos en espera de moto no coincide", numEsperando, pedMoto.size());   
   IList<String> motosDisp = gestor.getCodMotosDisponibles();
   assertEquals("El n�mero de motos disponibles no coincide", numDisponibles, motosDisp.size());   
   
   gestor.asignarPedido(pedidosGrandes[i]);
   
   IList<String[]> pedFurgo = gestor.getCodEsperandoFurgo(); 
   assertEquals("El n�mero de pedidos en espera de furgoneta no coincide", numEsperando, pedFurgo.size());
   IList<String> fusgosDisp = gestor.getCodFurgoDisponibles(); 
   assertEquals("El n�mero de furgonetas disponibles no coincide", numDisponibles, fusgosDisp.size());   
   
   // hay un pedido de cada tipo pendiente
   if (i>=3){
    pedMoto = gestor.getCodEsperandoMoto();
    assertEquals("El n�mero de pedidos en espera de moto no coincide", 1, pedMoto.size());
    
    gestor.notificarEntregaPedido(pedidosPequenos[i-3]);
    pedMoto = gestor.getCodEsperandoMoto(); 
    assertEquals("El n�mero de pedidos en espera de moto no coincide", 0, pedMoto.size());   
    pedFurgo = gestor.getCodEsperandoFurgo(); 
    assertEquals("El n�mero de pedidos en espera de furgoneta no coincide", 1, pedFurgo.size());
  ...
  // se notifican los �ltimos pedidos grandes
  ...
  IList<String> fusgosDisp = gestor.getCodFurgoDisponibles(); 
  assertEquals("El n�mero de furgonetas disponibles no coincide", 3, fusgosDisp.size()); 
  
  // se notifican los �ltimos pedidos peque�os
  ...
  IList<String> motosDisp = gestor.getCodMotosDisponibles();
  assertEquals("El n�mero de motos disponibles no coincide", 3, motosDisp.size());
 }
 /**
  * Test de notificarEntregaPedido de GestionRepartoLocal.
  * 
  ...
  */  
 @Test
 public void testNotificarEntregaPedido3() throws PedidoSinTransporteAsignado {
  final double PESOMAXMOTO = 20;
  Mapa mapa = new Mapa(200, 200);
  ...
  IList<String> fusgosDisp = gestor.getCodFurgoDisponibles(); 
  assertEquals("El n�mero de furgonetas disponibles no coincide", 0, fusgosDisp.size()); 
  ...
  IList<String> motosDisp = gestor.getCodMotosDisponibles();
  assertEquals("El n�mero de motos disponibles no coincide", 1, motosDisp.size());
